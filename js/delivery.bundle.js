...
var isPhone = window.matchMedia("(max-width: 767px)").matches ? true : false; // Move map in phone to first tab block
  ...
  arrTitles.forEach(function (topTitle) {
    topTitle.addEventListener('click', function () {
      ...
      if (isPhone && parent.classList.contains('active')) {
        ...
        document.documentElement.scrollTop = offsetTop - header.clientHeight - 30;
      } // end
      // Move map in phone to active tab block
...
function loadMap() {
  ...
  script.async = true;
  script.src = "https://api-maps.yandex.ru/2.1/?lang=ru_RU&amp;apikey=9fb45093-1306-4dc4-a928-84873cbf6687";

  script.onload = function () {
    initMap(); // Call by function because (ymaps) variable not exist yet
  };
...
function initMap() {
  ...
  function init() {
    ...
    var ButtonLayout = ymaps.templateLayoutFactory.createClass(['<div class="map-custom-buttons">', '<span class="map-custom-buttons__btn active js-map-custom-buttons__btn" data-map-type="yandex#map">$[data.firstButtonText]</span>', '<span class="map-custom-buttons__btn js-map-custom-buttons__btn" data-map-type="yandex#satellite">$[data.secondButtonText]</span>', '</div>'].join(''));
    var button = new ymaps.control.Button({
      data: {
        firstButtonText: "TFS Map",
        secondButtonText: "Спутник"
      },
      options: {
        layout: ButtonLayout
      }
    ...
    button.events.add('click', function (e) {
      var buttons = document.querySelectorAll('.js-map-custom-buttons__btn');
      Array.from(buttons).forEach(function (btn) {
        if (e.get('domEvent').get('target') === btn) {
          var mapType = btn.dataset.mapType;
          if (mapType) map.setType(mapType);
    ...
    // Change cordinates after clicked by address

    Array.from(cities).forEach(function (city) {
      city.addEventListener('click', function () {
        var cordinate = city.dataset.cordinatesOfMap.split(",");
        map.setCenter(cordinate);
        myPlacemark.geometry.setCoordinates(cordinate);
      });
    });
  }
} // end
...
/******/ 			return "js/" + chunkId + "." + "fd14ae122d9b0fe227b0" + ".js";
/******/ 		};
/******/ 	})();
/******/ 	
...
(() => {
/*!****************************************************!*\
  !*** ./templates/default/src/js/pages/delivery.js ***!
  \****************************************************/
